@component("components.layout.appshell", [
    ...
    <h1 class="mb-2">
        @if(!empty($user))
            {{ t("Edit administrator") }} „{{ $user->getUsername() }}”
        @else
            {{ t("Create administrator") }}
        @endif
    </h1>

    <form method="post" action="{{ Router::generate("administrators-save") }}">
        @if(!empty($user))
        ...
        @if(!empty($user))
            <button type="button"
                    ...
                    data-delete-href="{{ Router::generate("administrators-delete", ["userId" => $user->getId()]) }}">
