@auth
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 my-2">
        ...
        <div class="flex items-center gap-2">
            <div class="flex items-center justify-center shrink-0 w-12 h-12 bg-secondary rounded-full">
                <svg class="w-2/3 h-2/3 fill-secondary-font" xmlns="http://www.w3.org/2000/svg" viewBox="0 -960 960 960">
            ...
            <div class="flex flex-col break-all">
                <span class="text-sm">{{ t("Logged in as") }}</span>
                <span class="text-primary">{{ Auth::getLoggedInUser()?->getUsername() }}</span>
            </div>
        </div>
